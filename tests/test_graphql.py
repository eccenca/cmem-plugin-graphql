...
def test_execution(setup):
    ...
    plugin: GraphQLPlugin = GraphQLPlugin(graphql_url=GRAPHQL_URL, graphql_query=query, graphql_dataset=DATESET_ID)
    plugin.execute()
...
def test_validate_invalid_inputs():
    ...
    with pytest.raises(ValueError, match="Provide a valid GraphQL URL."):
        GraphQLPlugin(graphql_url=invalid_url, graphql_query=query, graphql_dataset=DATESET_ID)

    # Invalid query
    with pytest.raises(ValueError, match="Query string is not Valid"):
        GraphQLPlugin(graphql_url=GRAPHQL_URL, graphql_query=invalid_query, graphql_dataset=DATESET_ID)

    # Invalid Dateset
    with pytest.raises(ValueError, match="None is not a valid task ID."):
        GraphQLPlugin(graphql_url=GRAPHQL_URL, graphql_query=query, graphql_dataset="None")
